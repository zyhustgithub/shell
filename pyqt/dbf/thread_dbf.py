...
class DbfBreak:
    ...
    def Break(self, outpath, if_break=True, **kwargs):
        sem.acquire() 
        if if_break:
            if not kwargs:
...
if __name__ == '__main__':
    ...
    threads = []
    for i in range(20):
        t = threading.Thread(target=res.Break, args=('file/breack'+str(i)+'.dbf',), kwargs={'gbrq1':None, 'gbje1':0})
        t.start()
        threads.append(t)
    for t in threads:
        t.join()
    print('多线程完成时间：', time.time()-current)
    print('Finished')
    #print('finish loaded')
